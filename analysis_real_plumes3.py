...
model_ls = ['-', ':', '--', '-.']
model_lw = [lw, lw-1, lw, lw-1]

b_max = b_maxs[0]
n_seeds = np.size(seeds)
n_rhos = np.size(rhos)
n_wmax = np.size(w_maxs)
n_inhs = len(nsi_ln_par)
...
od_avg1 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
od_avg2 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
orn_avg1 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
orn_avg2 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_avg1 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_avg2 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))

# performance thrs2plot measures
pn_m50_1 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_m50_2 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_m100_1 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_m100_2 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_m150_1 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_m150_2 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))

pn_av50_1 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_av100_1 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_av150_1 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_av50_2 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_av100_2 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
pn_av150_2 = np.zeros((n_rhos, n_inhs, n_seeds, n_wmax,))
...
if avg_fig:
    ...
    for id_inh in range(4):
        ax_orn.errorbar(corr_obs, np.squeeze(np.mean(orn_avg1[:, id_inh, :, w_max_id,], axis=1)),
                  yerr=np.squeeze(np.std(orn_avg1[:, id_inh, :, w_max_id,], axis=1))/
                  (np.size(orn_avg1[:, id_inh, :, w_max_id,],axis=1))**.5, 
                  linewidth= model_lw[id_inh], ls = model_ls[id_inh], color=model_colours[id_inh], 
                  label = model_names[id_inh], fmt='o')
        
        ax_pn.errorbar(corr_obs, np.squeeze(np.mean(pn_avg1[:, id_inh, :, w_max_id,], axis=1)),
                  yerr=np.squeeze(np.std(pn_avg1[:, id_inh, :, w_max_id,], axis=1))/
                  (np.size(pn_avg1[:, id_inh, :, w_max_id,],axis=1))**.5, 
                  linewidth= model_lw[id_inh], ls = model_ls[id_inh], color=model_colours[id_inh], 
                  label = model_names[id_inh], fmt='o')

        y2plot_tmp = np.squeeze(pn_tmp[:, id_inh, :, w_max_id, ])
        
        ax_peak.errorbar(corr_obs, np.mean(y2plot_tmp, axis=1),
                yerr=np.std(y2plot_tmp, axis=1)/np.sqrt(n_seeds),
                linewidth=model_lw[id_inh], ls=model_ls[id_inh], color=model_colours[id_inh], 
    ...
    ax_pn.set_xticks([0, .5, 1])
    ax_orn.set_xticks([0, .5, 1])
    ax_peak.set_xticks([0, .5, 1])
    ...
    ax_orn.legend(fontsize=label_fs, frameon=False, loc=[0.5, .35])
...
if peak_fig:
    ...
    for thr_id, thr in enumerate(thrs2plot):
        ...
        for id_inh in range(4):
            ...
            axs[thr_id].errorbar(corr_obs, np.mean(y2plot_tmp, axis=1),
               yerr=np.std(y2plot_tmp, axis=1)/np.sqrt(n_seeds),
               linewidth=model_lw[id_inh], color=model_colours[id_inh], 
               ls= model_ls[id_inh], label=model_names[id_inh])
    ...
    for thr in thrs2plot:
        ...
        for w_max_id, w_max in enumerate(w_maxs):
            ...
            for id_inh in range(4):
                ...
                axs[w_max_id].errorbar(corr_tmp, np.mean(y2plot_tmp, axis=1),
                   yerr=np.std(y2plot_tmp, axis=1)/np.sqrt(n_seeds),
                   linewidth=model_lw[id_inh], color=model_colours[id_inh], 
                   ls=model_ls[id_inh], label=model_names[id_inh])
...
if resumen_fig:
    ...
    for thr_id, thr in enumerate(thrs2plot):
        ...
        for id_inh in [1,2,3,]:
            delta_0 = np.squeeze(pn_tmp0[0,:] - pn_tmp0[id_inh,:])
            delta_10 = np.squeeze(pn_tmp0[id_inh,:] - pn_tmp1[id_inh,:])        
            delta_0_err = np.sqrt(pn_tmp_err[0,:] + pn_tmp_err[id_inh,:])/np.sqrt(n_seeds)        
            delta_10_err = np.sqrt(pn_tmp_err[id_inh,:] + pn_tmp_err[id_inh,:])/np.sqrt(n_seeds)
                        
            axs[0].errorbar(w_maxs, delta_0, yerr= delta_0_err, linewidth=model_lw[id_inh], 
                            linestyle=model_ls[id_inh], color=model_colours[id_inh], 
                            label=r'$x$='+model_names[id_inh]+', $\rho=$0')
        
            axs[1].errorbar(w_maxs, delta_10, yerr= delta_10_err, linewidth=model_lw[id_inh], 
                            linestyle=model_ls[id_inh], color=model_colours[id_inh], 
                            label=r'$x$='+model_names[id_inh]+', $\rho=$1')
        
            delta_0_max_tmp = np.max(delta_0)
            delta_0_max = np.max([delta_0_max_tmp,delta_0_max])
...
#            axs[id_col].tick_params(axis='both', which='major', labelsize=label_fs)
        ...
        axs[0].text(.03, delta_0_max*.9, 'x=NSI', color=cyan, fontsize=label_fs)
        axs[0].text(.03, delta_0_max*.8, 'x=mix', color=green, fontsize=label_fs)
        axs[0].text(.03, delta_0_max*.7, 'x=LN', color=orange, fontsize=label_fs)
