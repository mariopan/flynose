...
pn_ln_params = dict([
                    # CHANGED params
                    ('vrev_pn',     5*15),    # 15 [mV] reversal potential

                    ('vrest_pn',  -1*6.5),    # -6.5 [mV] resting potential
                    ('tau_s',       2* 10),    # 10 [ms]
                    ...
                    ('vpn_noise',  .061*10),  # NEW # extra noise input to PNs
...
peak_ratio                  = 1
peaks                       = [1e-4, 2e-4, 5e-4, 1e-2]#*np.logspace(-4, -2.5, 10)  # np.array([3e-4, 0.0006,0.0012, 0.0025,])#.005])#np.array([0.0001, 0.0006,0.0012, 0.0025, 0.005]) # np.logspace(-4, -1, 5) 
