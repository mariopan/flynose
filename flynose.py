...
def main(params2an, verbose=False, fld_analysis='', stim_seed=0, fig_ui=True):
    ...
    t2simulate      = np.maximum(params2an[2],2000) # [ms]
    ...
    cor_start       = 0.4 # Starting corr between ORNs
    ...
    if stimulus == 'ss':
        ...
        delay2an        = params2an[3] # 0
        if verbose:
            print('Stimulus: Single Step')
            print('Stimuli duration:      ' + '%.0f'%dur2an + ' ms')
            print('Delay between stimuli:    ' + '%.0f'%delay2an + ' ms')
...
#        interm_est[rr,1] = np.sum(out_w>0)/(t2sim*sample_rate)
    ...
    mv_cov_tmp  = ((1-cor_start)*np.identity(num_orns_glo) +
                 cor_start*np.ones((num_orns_glo, num_orns_glo))) # diagonal covariance
    ...
    if save_data:
        ...
        output_names = ['orn_sdf_norm', 'orn_sdf_time', ]        
        ...
        with open(fld_analysis+name_data[0], 'wb') as f:
            pickle.dump([params2an, orn_sdf_norm, orn_sdf_time, 
    ...
    if orn_fig:
        ...
        t2plot = -t_on, t2simulate-t_on
        rs = 4 # number of rows
        cs = 1 # number of cols
        
        fig_orn = plt.figure(figsize=[6, 8])
...
#        fig_orn.tight_layout()
        ...
        ax_orn1.plot(t-t_on, u_od[:,0], color= orange,linewidth=lw+1, 
                     label=r'Glom %d'%(1))
        ax_orn1.plot(t-t_on, u_od[:,1], '--', color=col_glo[1,:],linewidth=lw, 
                     label=r'Glom %d'%(2)) # before it was multiplied by 100
        ax_orn1.set_ylabel(r'Odor concentration', fontsize=label_fs)
        ax_orn1.legend(loc=4)
        ax_orn1.set_title('ORN dynamics', fontsize=title_fs)
        ...
        ax_orn3.plot(t-t_on, x_orn[:,0], color=orange,linewidth=lw+1,
                     label=r'Od, glom : %d'%(0))
        ax_orn3.plot(t-t_on, x_orn[:,1], '--',color=col_glo[1,:],linewidth=lw, 
                     label=r'Od, glom : %d'%(1))
        ...
        ax_orn_fr.set_ylabel(r'firing rates (Hz)', fontsize=label_fs)
        
        ax_orn_fr.plot(orn_sdf_time-t_on, np.mean(orn_sdf_norm[:,:num_orns_glo], axis=1)*1e3, 
                     color=orange,  linewidth=lw+1,label='sdf glo 1')
        ax_orn_fr.plot(orn_sdf_time-t_on, np.mean(orn_sdf_norm[:,num_orns_glo:], axis=1)*1e3, 
                     color=col_glo[1,:], linewidth=lw,label='sdf glo 2')
        ...
        ax_orn_sc.scatter(spikes_orn_0[:,0]/pts_ms-t_on, 
                        spikes_orn_0[:,1], color=col_glo[1,:], s=10)
        ax_orn_sc.scatter(spikes_orn_1[:,0]/pts_ms-t_on, 
                        num_orns_glo+spikes_orn_1[:,1], color=orange, s=10)
...
#    cmn_params  = net_params[4]
    ...
    od_avg1 = np.mean(u_od[stim_on:stim_off, 0], axis=0)
    od_avg2 = np.mean(u_od[stim_on:stim_off, 1], axis=0)

    orn_avg1 = np.mean(nu_orn_run[stim_on:stim_off, 0], axis=0)
    orn_avg2 = np.mean(nu_orn_run[stim_on:stim_off, 1], axis=0)
...
#            '_wmax_%.2g'%(params2an[8])]
    ...
    # FIGURE ORN, PN, LN
    if al_dyn & al_fig:
        t2plot = -100, 300, #000, t2simulate#250, 1000 #
        nu2plot = 0, 120
        rs = 4 # number of rows
        cs = 1 # number of cols
        
        fig_pn = plt.figure(figsize=[8, 12])
...
#        fig_pn.tight_layout()
        ...
        ax_orn.set_ylim(0,90)
        ax_pn.set_ylim(0,120)
        ax_ln.set_ylim(0,160)
...
#        ax_conc.set_title(r'Stim type: ' + params2an[7] + 
#                           r', nsi: %.1f'%(params2an[0]) +
#                           r', ln spike height: %.2g'%(params2an[1]) +
#                           r', peak:  %.1f '%(params2an[4]) +
#                           r', rho: $1-10^{-%d}$'%(params2an[6]) + 
#                           r', $w_{max}$: %d s'%(params2an[8]), 
#                           fontsize=title_fs)
        ...
        ax_conc.plot(t-t_on, 100*u_od[:,0], color=orange, linewidth=lw+2, 
                      label='glom : '+'%d'%(1))
        ax_conc.plot(t-t_on, 100*u_od[:,1], '--',color=col_glo[1,:], linewidth=lw+1, 
                      label='glom : '+'%d'%(2))
         
#        ax_orn.plot(t, nu_orn_run[:,0], color=orange, linewidth=lw, 
#                      label=' glom : '+'%d'%(1))
#        ax_orn.plot(t, nu_orn_run[:,1],'--', color=col_glo[1,:], linewidth=lw-2, 
#                      label=' glom : '+'%d'%(2))
        
        ax_orn.plot(orn_sdf_time-t_on, np.mean(orn_sdf_norm[:,:num_orns_glo], axis=1)*1e3, 
                     color=orange, linewidth=lw+1,label='sdf glo 1')
        ax_orn.plot(orn_sdf_time-t_on, np.mean(orn_sdf_norm[:,num_orns_glo:], axis=1)*1e3, 
                     '--',color=col_glo[1,:], linewidth=lw,label='sdf glo 2')
        ...
        for pp in range(num_pns_tot):
            if pp >= num_pns_glo:
#                ax_pn.plot(t-t_on, nu_pn_run[:,pp], color=col_glo[ids_pn_glo[pp],:], 
#                              linewidth=lw+1, label='PN : '+'%d'%(pp))
                ax_pn.plot(pn_sdf_time-t_on, pn_sdf_norm[:,pp]*1e3, '--',color=col_glo[ids_pn_glo[pp],:], 
                              linewidth=lw, label='PN : '+'%d'%(pp))
            else:
#                ax_pn.plot(t-t_on, nu_pn_run[:,pp],color=orange, 
#                              linewidth=lw, label='PN : '+'%d'%(pp))
                ax_pn.plot(pn_sdf_time-t_on, pn_sdf_norm[:,pp]*1e3, color=orange, 
                              linewidth=lw+1, label='PN : '+'%d'%(pp))
        
        for ll in range(num_lns_tot):
            if ll >= num_lns_glo:
#                ax_ln.plot(t-t_on, nu_ln_run[:,ll], color=col_glo[ids_ln_glo[ll],:], 
#                              linewidth=lw+1, label='LN : '+'%d'%(ll))
                ax_ln.plot(ln_sdf_time-t_on, ln_sdf_norm[:,ll]*1e3, '--',color=col_glo[ids_ln_glo[ll],:], 
                              linewidth=lw, label='LN : '+'%d'%(ll))
            else:
#                ax_ln.plot(t-t_on, nu_ln_run[:,ll],color=orange, 
#                              linewidth=lw, label='PN : '+'%d'%(ll))
                ax_ln.plot(ln_sdf_time-t_on, ln_sdf_norm[:,ll]*1e3, color=orange,
                              linewidth=lw+1, label='LN : '+'%d'%(ll))
            ...
            for w_max in [3]: # 3, 50, 150
                for rho in [0]: #[0, 1, 3, 5]: 
                    for inh_cond in inh_conds:
                        if inh_cond == 'nsi':
                            params2an = [.3, .0, stim_dur, delay2an, peak, peak_ratio, rho, stim_type,w_max,b_max]
                        elif inh_cond == 'noin':
                            params2an = [0, 0, stim_dur, delay2an, peak, peak_ratio, rho, stim_type,w_max,b_max]
                        elif inh_cond == 'ln':
                            params2an = [.0, .15, stim_dur, delay2an, peak, peak_ratio, rho, stim_type,w_max,b_max]
                        
                        print(params2an)
                        #    params2an = [nsi_value, ln_spike_height, dur2an, delay2an, peak, peak_ratio]
                        # fig_ui = False to close the plots
                        plt.ion()      # ion() # to avoid showing the plot every time     
                        tic = timeit.default_timer()
                        [pn_avg1, pn_avg2, ] = main(params2an, verbose = True, fld_analysis = fld_analysis, stim_seed=stim_seed, fig_ui=True)
                        print('pn avg 1:%.3g, pn avg 2:%.3g'%(pn_avg1,pn_avg2))
                        toc = timeit.default_timer()
                        print('time to run sim: %.0f'%(toc-tic))
                    print('')
