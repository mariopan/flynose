...
def main(params2an, fig_opts, verbose=False, fld_analysis='', stim_seed=0):
    ...
    t2simulate      = np.maximum(params2an[2],1500) # [ms]
...
#        interm_est[rr,1] = np.sum(out_w>0)/(t2sim*sample_rate)
    ...
    if orn_fig:  
        t2plot = -600, 900 #-t_on, t2simulate-t_on
        rs = 4 # number of rows
        cs = 1 # number of cols
        
        fig_orn = plt.figure(figsize=[7.5, 8])
...
#        fig_orn.tight_layout()
        ...
        ax_orn1.legend(loc='upper left', fontsize=label_fs)
        ...
        if fig_save:
            fig_orn.savefig(fld_analysis + 
                            '/images/ORN_t1_' + '%d'%(t_on-t_on) + '-' + '%d'%(t_off-t_on) + 
...
#    cmn_params  = net_params[4]
    ...
    pn_m50_2 = np.sum(np.mean(pn_sdf_norm[id_stim, num_pns_glo:], axis=1)>100)*pn_sdf_dt
    pn_m100_2 = np.sum(np.mean(pn_sdf_norm[id_stim, num_pns_glo:], axis=1)>100)*pn_sdf_dt
    pn_m150_2 = np.sum(np.mean(pn_sdf_norm[id_stim, num_pns_glo:], axis=1)>100)*pn_sdf_dt
    ...
    pn_peak2  = np.max(np.mean(pn_sdf_norm[id_post_stim, num_pns_glo:], axis=1)) # using average PN
...
#    pn_m150_2  =np.sum(np.mean(nu_pn_run[stim_on:stim_off, num_pns_glo:], axis=1)>150)
    ...
    orn_avg1 = np.mean(nu_orn[stim_on:stim_off, 0], axis=0)
    orn_avg2 = np.mean(nu_orn[stim_on:stim_off, 1], axis=0)

    # SAVE SDF OF conc, ORN, PN and LN FIRING RATE
    if data_save:
        name_data = ['/data/ORNALrate' +
                    '_stim_' + params2an[7] +
                    '_nsi_%.1f'%(params2an[0]) +
                    '_lnspH_%.2f'%(params2an[1]) +
                    '_dur2an_%d'%(params2an[2]) +
    ...
    if al_dyn & al_fig:
        ...
        t2plot = -100, 300 # 000-t_on, t2simulate-t_on
        rs = 4 # number of rows
        cs = 1 # number of cols
...
#        ax_conc.set_title(r'nsi: '+r'%.1g'%(nsi_value)+ ', '+
#                           r'ln spike height: '+r'%.2g'%(ln_spike_height)+', '+
#                           r'peak:  '+r'%.1f '%(peak1)+', '+
#                           r'peak ratio:  '+r'%.1f '%(peak2/peak1)+', '+
#                           r'stim duration:  '+r'%.0f ms'%(dur2an)+', '+
#                           r'delay:  '+r'%.0f ms'%(delays2an), fontsize=title_fs)
...
#        ax_orn.plot(t, nu_orn_run[:,0], color=green, linewidth=lw, 
#                      label=' glom : '+'%d'%(1))
#        ax_orn.plot(t, nu_orn_run[:,1],'--', color=purple, linewidth=lw-2, 
#                      label=' glom : '+'%d'%(2))
...
#                ax_pn.plot(t-t_on, nu_pn_run[:,pp],color=green, 
#                              linewidth=lw, label='PN : '+'%d'%(pp))
...
#                ax_ln.plot(t-t_on, nu_ln_run[:,ll],color=green, 
                ...
                for w_max in [3]: # 3, 50, 150
                    for rho in [0]: #[0, 1, 3, 5]: 
                        for inh_cond in inh_conds:
                            if inh_cond == 'nsi':
                                params2an = [.3, .0, stim_dur, delays2an, peak, 
                                             peak_ratio, rho, stim_type,w_max,b_max]
                            elif inh_cond == 'noin':
                                params2an = [0, 0, stim_dur, delays2an, peak, 
                                             peak_ratio, rho, stim_type,w_max,b_max]
                            elif inh_cond == 'ln':
                                params2an = [.0, .15, stim_dur, delays2an, peak, 
                                             peak_ratio, rho, stim_type,w_max,b_max]
                            ...
                            [pn_peak1, pn_peak2, ]  = main(params2an, fig_opts, verbose = False, 
                            fld_analysis = fld_analysis, stim_seed=stim_seed)
                            
                            print('pn peak 1:%.3g, pn peak 2:%.3g'%(pn_peak1, pn_peak2 ))
